...
def main():
    ...
    print(f'│ {"Base":^5} │ {"Conf":^5} │ {"ExtClass":^10} │'
          f' {"Emb Sim":>8} │ {"ExtEmb":>8} │ {"GrpEmb":>8} │'
          f' {"Head Sim":>8} │ {"ExtHead":>8} │ {"GrpHead":>8} │')
    print(sep)
    ...
    for ch in sorted(grouped.keys()):
        ...
        for i, p in enumerate(grouped[ch]):
            ...
            # Show group avg only on first row of each base char
            if i == 0:
            ...
            print(f'│{prefix}│ {confused_char:^5} │ {p["ext_name"]:<10} │'
                  f' {emb_sim:>8.4f} │ {ext_emb_str} │ {grp_emb_str} │'
                  f' {head_str} │ {ext_head_str} │ {grp_head_str} │')
    ...
    print(f'\nCosine Similarity Statistics (n={len(results)}):')
    print(f'  {"":20} {"Mean":>8} {"Std":>8} {"Min":>8} {"Max":>8} {"Median":>8}')
    for label, sims in [('Emb Sim', emb_sims), ('Head Sim', head_sims),
