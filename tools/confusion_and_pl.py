...
def build_confusion_matrix(model, data_root, train_dirs, charset, device, batch_size=512, num_workers=4):
    """
    ...
    for train_dir in train_dirs:
        train_root = Path(data_root) / 'train' / train_dir
        lmdb_dirs = sorted(
            Path(p).parent
        ...
        for lmdb_dir in lmdb_dirs:
            ...
            print(f'Processing train/{train_dir}/{ds_name} ({len(dataset)} samples)...')
...
def main():
    ...
    # ==================== Step 1 ====================
    print('\n' + '=' * 60)
    print(f'Step 1: Building confusion matrix from train data ({", ".join(args.train_dirs)})')
    print('=' * 60)

    confusion = build_confusion_matrix(
        model, args.data_root, args.train_dirs, charset, args.device,
