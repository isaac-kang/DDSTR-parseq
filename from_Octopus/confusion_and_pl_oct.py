...
def main():
    ...
    # ── Unified Table: Class / ExtClass / Confusion Rate / Subtotal ──
    lines.append(f'Confusion & Extended Class Summary  (threshold >= {args.min_rate*100:.1f}%)')
    lines.append('=' * 110)
    hdr = (f'│ {"Class":^5} │ {"Total":>7} │ {"Acc(%)":>8} │ {"Err(%)":>8} │'
           f' {"ExtClass":^10} │ {"->Confused":^10} │ {"Count":>7} │ {"Rate(%)":>9} │')
    sep = '├' + '─' * 7 + '┼' + '─' * 9 + '┼' + '─' * 10 + '┼' + '─' * 10 + '┼' + '─' * 12 + '┼' + '─' * 12 + '┼' + '─' * 9 + '┼' + '─' * 11 + '┤'
    top = '┌' + '─' * 7 + '┬' + '─' * 9 + '┬' + '─' * 10 + '┬' + '─' * 10 + '┬' + '─' * 12 + '┬' + '─' * 12 + '┬' + '─' * 9 + '┬' + '─' * 11 + '┐'
    bot = '└' + '─' * 7 + '┴' + '─' * 9 + '┴' + '─' * 10 + '┴' + '─' * 10 + '┴' + '─' * 12 + '┴' + '─' * 12 + '┴' + '─' * 9 + '┴' + '─' * 11 + '┘'
    lines.append(top)
    lines.append(hdr)
    lines.append(sep)
    ...
    for ch in sorted(charset):
        if ch not in confusion:
            continue
        total_samples = sum(confusion[ch].values())
        correct = confusion[ch].get(ch, 0)
        acc = correct / total_samples * 100 if total_samples else 0
        err = 100 - acc

        if ch in confusion_detail:
            d = confusion_detail[ch]
            confused_list = d['confused']
            n_ext = len(confused_list)
            sum_rate = sum(t['rate'] for t in confused_list) * 100

            for i, t in enumerate(confused_list):
                ext_name = d['extended_classes'][i + 1]
                total_extended += 1
                grand_total_ext += 1
                if i == 0:
                    cls_col = f' {"":1}{ch:^3}{"":1} '
                    lines.append(f'│{cls_col}│ {total_samples:>7} │ {acc:>7.2f}% │ {err:>7.2f}% │'
                                 f' {ext_name:<10} │ {t["char"]:^10} │ {t["count"]:>7} │ {t["rate"]*100:>8.3f}% │')
                else:
                    lines.append(f'│ {"":5} │ {"":7} │ {"":8} │ {"":8} │'
                                 f' {ext_name:<10} │ {t["char"]:^10} │ {t["count"]:>7} │ {t["rate"]*100:>8.3f}% │')

            # Subtotal row for this class
            lines.append(f'│ {"":5} │ {"":7} │ {"":8} │ {"":8} │'
                         f' {"(sum)":>10} │ {"":10} │ {"":7} │ {sum_rate:>8.3f}% │')
            lines.append(sep)
        else:
            # Class with no extended classes
            lines.append(f'│  {ch:^3}  │ {total_samples:>7} │ {acc:>7.2f}% │ {err:>7.2f}% │'
                         f' {"—":^10} │ {"—":^10} │ {"—":>7} │ {"—":>9} │')
            lines.append(sep)
    ...
    # Extended class confusion rate statistics
    all_ext_rates = []
    for ch in sorted(confusion_detail.keys()):
        d = confusion_detail[ch]
        for t in d['confused']:
            all_ext_rates.append(t['rate'] * 100)
    ext_rates_arr = np.array(all_ext_rates)
    lines.append(f'Extended class confusion rate stats (n={len(ext_rates_arr)}):')
    lines.append(f'  Mean:   {ext_rates_arr.mean():>8.3f}%')
    lines.append(f'  Std:    {ext_rates_arr.std():>8.3f}%')
    lines.append(f'  Median: {np.median(ext_rates_arr):>8.3f}%')
    lines.append(f'  Min:    {ext_rates_arr.min():>8.3f}%')
    lines.append(f'  Max:    {ext_rates_arr.max():>8.3f}%')
    lines.append('')
    lines.append(f'Original charset size: {len(charset)}')
    lines.append(f'Characters with extended classes: {len(confusion_detail)}')
    lines.append(f'Total extended classes added: {total_extended}')
